...
RSpec.describe Attraction, :type => :model do
  ...
  it "has many rides" do
    user = User.create(name: "Max Charles", password: "password", nausea: 5, happiness: 3, tickets: 4, height: 34)
    ride = Ride.create(user_id: user.id, attraction_id: attraction.id)
    expect(attraction.rides.first).to eq(ride)
  end

  it "has many users through rides" do
    max = User.create(name: "Max Charles", password: "password", nausea: 5, happiness: 3, tickets: 4, height: 34)
    skai = User.create(name: "Skai Jackson", password: "password", nausea: 5, happiness: 3, tickets: 4, height: 34)
